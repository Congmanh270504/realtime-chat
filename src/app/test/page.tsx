...
// // user_31dlsNLyd3Xq95UloZjtmr8HkE0;
"use client";
...
const Page = () => {
  const searchUser = async () => {
    const response = await fetch(
      `/api/friends/getFriends/?userId=user_31dlsNLyd3Xq95UloZjtmr8HkE0`
    );
    if (!response.ok) throw new Error("Network response was not ok");
    return response.json();
  };
  const { data } = useQuery({ queryKey: ["searchUser"], queryFn: searchUser });
